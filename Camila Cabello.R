...
b <- 14 #how many variables do you have (songs)
...
mydf14 <- data_frame(line=1, text=my_txt14)
...
frequencies_tokens_nostop13 <- mydf13 %>%
  ...
  anti_join(stop_words) %>% #here's where we remove tokens
  count(word, sort=TRUE)
frequencies_tokens_nostop14 <- mydf14 %>%
  unnest_tokens(word, text) %>%
  anti_join(stop_words) %>% #here's where we remove tokens
  count(word, sort=TRUE)
...
#TFIDF
combined_c <-bind_rows(
  mutate(frequencies_tokens_nostop1, song = "Shameless"),
  mutate(frequencies_tokens_nostop2, song = "Living Proof"),
  mutate(frequencies_tokens_nostop3, song = "Should've Said it"),
  mutate(frequencies_tokens_nostop4, song = "My Oh My"),
  mutate(frequencies_tokens_nostop5, song = "Senorita"),
  mutate(frequencies_tokens_nostop6, song = "Liar"),
  mutate(frequencies_tokens_nostop7, song = "Bad Kind of Butterflies"),
  mutate(frequencies_tokens_nostop8, song = "Easy"),
  mutate(frequencies_tokens_nostop9, song = "Feel it Twice"),
  mutate(frequencies_tokens_nostop10, song = "Dream of You"),
  mutate(frequencies_tokens_nostop11, song = "Cry for Me"),
  mutate(frequencies_tokens_nostop12, song = "This Love"),
  mutate(frequencies_tokens_nostop13, song = "Used to This"),
  mutate(frequencies_tokens_nostop14, song = "First Man"),
)

camila_combined <- combined_c%>%
